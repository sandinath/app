...
function getStorageContent(req, res, pathToServe, matchedRoute, responseOverridesRule, navigationFallbackRule, mimeTypeRule, routeHeaders, globalHeaders) {
    ...
    if (core_1.isSWAConfigFileUrl(req)) {
        core_1.logger.silly(` - request to config file detected. Exit`);
        error_page_handler_1.handleErrorPage(req, res, 404, responseOverridesRule);
        return {
            isFunctionFallbackRequest: false,
            isSuccessfulSiteHit: false,
        };
    }
    const { matchingRewriteRoutePath } = route_processor_1.parseQueryParams(req, matchedRoute);
    ...
    let filePathFromRequest = routes_engine_1.tryFindFileForRequest(requestPath);
    ...
    if (!filePathFromRequest) {
        ...
        if (navigationFallbackRule === null || navigationFallbackRule === void 0 ? void 0 : navigationFallbackRule.rewrite) {
            const isFunctionFallback = function_handler_1.isFunctionRequest(req, navigationFallbackRule.rewrite);
            ...
            if (isFunctionFallback) {
                ...
                const isNavigationFallbackWritePathExists = routes_engine_1.tryFindFileForRequest(navigationFallbackRewrite);
                if (isNavigationFallbackWritePathExists &&
                    !routes_engine_1.isRequestPathExcludedFromNavigationFallback(decodedRequestPath, navigationFallbackRule, matchedRoute)) {
    ...
    if (core_1.isHttpUrl(filePathFromRequest)) {
        return {
            isFunctionFallbackRequest: false,
    ...
    // mime type
    const mimeType = routes_engine_1.getMimeTypeForExtension(filePathFromRequest, mimeTypeRule);
    res.setHeader("Content-Type", mimeType);
    // compute both global and route headers
    const matchingRouteHeaders = routes_engine_1.getHeadersForRoute(routeHeaders, globalHeaders);
    if (responseOverridesRule) {
        // Handle HEAD request
        if (req.method === "HEAD") {
            routes_engine_1.updateResponseHeaders(res, matchingRouteHeaders);
            res.statusCode = 200;
            return {
                isFunctionFallbackRequest: false,
                isSuccessfulSiteHit: true,
