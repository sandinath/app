...
function detectTargetCoreToolsVersion(nodeVersion) {
    ...
    if (nodeVersion >= 14 && nodeVersion <= 16)
        return 4;
...
async function getInstalledSystemCoreToolsVersion() {
    try {
        const { stdout: version } = await util_1.promisify(child_process_1.exec)(`${DEFAULT_FUNC_BINARY} --version`);
...
async function getLatestCoreToolsRelease(targetVersion) {
    try {
        const response = await node_fetch_1.default(RELEASES_FEED_URL);
...
async function downloadAndUnzipPackage(release, dest) {
    const response = await node_fetch_1.default(release.url);
    ...
    const bodyStream1 = response.body.pipe(new stream_1.PassThrough());
    const bodyStream2 = response.body.pipe(new stream_1.PassThrough());
