...
function handleErrorPage(req, res, statusCode, responseOverridesRule) {
    ...
    if (responseOverridesRule) {
        routes_engine_1.responseOverrides(req, res, responseOverridesRule);
    }
    if (!route_processor_1.isCustomUrl(req)) {
        req.url = `/${statusCode}.html`;
    }
    core_1.logRequest(req, constants_1.SWA_CLI_APP_PROTOCOL + "://" + req.headers.host, statusCode);
}
