...
async function handleUserConfig(appLocation) {
    ...
    try {
        ...
        core_1.logger.silly(`${chalk_1.default.red("configuration file is invalid!")}`);
        core_1.logger.silly(`${chalk_1.default.red(error.toString())}`);
    }
    return configJson;
}
...
function serveStaticOrProxyResponse(req, res, proxyApp, target) {
    ...
    const customUrl = route_processor_1.isCustomUrl(req);
    ...
    if (((_a = req.url) === null || _a === void 0 ? void 0 : _a.includes("index.html")) || customUrl) {
        ...
        req.url = (_b = req.url) === null || _b === void 0 ? void 0 : _b.replace(config_1.DEFAULT_CONFIG.customUrlScheme, "");
        target = constants_1.SWA_CLI_OUTPUT_LOCATION;
    ...
    if (constants_1.IS_APP_DEV_SERVER() && (!is4xx || customUrl)) {
        ...
        proxyApp.once("proxyRes", (proxyRes) => {
            ...
            core_1.logRequest(req, target, proxyRes.statusCode);
        });
    }
    else {
        // not a dev server
        ...
        let file = null;
        target = constants_1.SWA_CLI_OUTPUT_LOCATION;
        const fileInOutputLocation = path_1.default.join(target, req.url);
        const existsInOutputLocation = fs_1.default.existsSync(fileInOutputLocation);
        ...
        const done = finalhandler_1.default(req, res, { onerror });
        // serving static content is only possible for GET requests
        req.method = "GET";
        serve_static_1.default(target, { extensions: ["html"] })(req, res, done);
    }
}
/**
...
async function requestMiddleware(req, res, proxyApp, userConfig) {
    ...
    if (isWebsocketRequest(req)) {
        ...
        return serveStaticOrProxyResponse(req, res, proxyApp, constants_1.SWA_CLI_OUTPUT_LOCATION);
    }
    let target = constants_1.SWA_CLI_OUTPUT_LOCATION;
    core_1.logger.silly(`checking for matching route`);
    const matchingRouteRule = routes_engine_1.tryGetMatchingRoute(req, userConfig);
    ...
    const isAuthReq = auth_handler_1.isAuthRequest(req);
    ...
    const isFunctionReq = function_handler_1.isFunctionRequest(req, matchingRouteRule === null || matchingRouteRule === void 0 ? void 0 : matchingRouteRule.rewrite);
    if (!isFunctionReq) {
        core_1.logger.silly(` - not a function request`);
    }
    if (!routes_engine_1.isRequestMethodValid(req, isFunctionReq, isAuthReq)) {
        res.statusCode = 405;
        return res.end();
    }
    core_1.logger.silly(`checking for query params`);
    const { matchingRewriteRoutePath, sanitizedUrl, matchingRewriteRoute } = route_processor_1.parseQueryParams(req, matchingRouteRule);
    core_1.logger.silly(`checking rewrite auth login request`);
    if (matchingRewriteRoute && auth_handler_1.isLoginRequest(matchingRewriteRoutePath)) {
    ...
    if (matchingRewriteRoute && auth_handler_1.isLogoutRequest(matchingRewriteRoutePath)) {
        core_1.logger.silly(` - auth logout dectected`);
        authStatus = constants_1.AUTH_STATUS.HostNameAuthLogout;
        req.url = sanitizedUrl.toString();
        return await auth_handler_1.handleAuthRequest(req, res, matchingRouteRule, userConfig);
    }
    if (!routes_engine_1.isRouteRequiringUserRolesCheck(req, matchingRouteRule, isFunctionReq, authStatus)) {
        error_page_handler_1.handleErrorPage(req, res, 401, userConfig === null || userConfig === void 0 ? void 0 : userConfig.responseOverrides);
        return serveStaticOrProxyResponse(req, res, proxyApp, target);
    }
    if (authStatus != constants_1.AUTH_STATUS.NoAuth && (authStatus != constants_1.AUTH_STATUS.HostNameAuthLogin || !matchingRewriteRoute)) {
        if (authStatus == constants_1.AUTH_STATUS.HostNameAuthLogin && matchingRouteRule) {
            return auth_handler_1.getAuthBlockResponse(req, res, userConfig, matchingRouteRule);
        }
        return await auth_handler_1.handleAuthRequest(req, res, matchingRouteRule, userConfig);
    }
